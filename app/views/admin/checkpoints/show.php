<?php
/**
 * ADMIN - CHI TIẾT CHECKPOINT (CHỈ XEM)
...
<div class="max-w-6xl mx-auto p-4 lg:p-8">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4 lg:mb-6">
        <div>
            <h1 class="text-xl lg:text-2xl font-bold text-primary-700">Chi tiết Checkpoint</h1>
            <p class="text-xs lg:text-sm text-primary-500 mt-1">
                <?= htmlspecialchars($checkpoint['checkpoint_name']) ?> - <?= htmlspecialchars($tour['tour_code']) ?>
            </p>
        </div>
    ...
    <div class="bg-panel rounded-2xl p-4 lg:p-6 mb-4 lg:mb-6 border border-primary-100">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
            <div>
                <span class="text-primary-500 block mb-1">Tên:</span>
                <div class="font-semibold text-primary-700"><?= htmlspecialchars($checkpoint['checkpoint_name']) ?></div>
            </div>
            <div>
                <span class="text-primary-500 block mb-1">Loại:</span>
                <div class="font-semibold text-primary-700"><?= ucfirst($checkpoint['checkpoint_type']) ?></div>
            </div>
            <div>
                <span class="text-primary-500 block mb-1">Ngày:</span>
                <div class="font-semibold text-primary-700"><?= date('d/m/Y', strtotime($checkpoint['scheduled_date'])) ?></div>
            </div>
            <div>
                <span class="text-primary-500 block mb-1">Thời gian:</span>
                <div class="font-semibold text-primary-700"><?= $checkpoint['scheduled_time'] ? date('H:i', strtotime($checkpoint['scheduled_time'])) : 'N/A' ?></div>
            </div>
            <div>
                <span class="text-primary-500 block mb-1">Địa điểm:</span>
                <div class="font-semibold text-primary-700"><?= htmlspecialchars($checkpoint['location_name'] ?? 'N/A') ?></div>
            </div>
            <div>
                <span class="text-primary-500 block mb-1">HDV tạo:</span>
                <div class="font-semibold text-primary-700"><?= htmlspecialchars($checkpoint['created_by_name'] ?? 'N/A') ?></div>
            </div>
        </div>
    </div>
                        ...
                        <tbody class="divide-y divide-primary-100">
                            <?php foreach ($checkins as $checkin): ?>
                                <tr class="hover:bg-primary-50 transition-colors">
                                    ...
                                    <td class="px-3 lg:px-4 py-2 lg:py-3">
                                        <?php
                                        $statusColors = ['present' => 'success', 'absent' => 'danger', 'late' => 'warning', 'early' => 'info', 'excused' => 'accent'];
                                        $statusTexts = ['present' => 'Có mặt', 'absent' => 'Vắng mặt', 'late' => 'Muộn', 'early' => 'Sớm', 'excused' => 'Miễn'];
