...
<div class=" mx-auto">
    ...
    <div class="bg-panel p-3 lg:p-4 rounded-2xl mb-4 border border-primary-100 shadow-sm">
        <form method="GET" id="filterForm">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 lg:gap-4">
                <div>
                    <input type="text" id="searchInput" placeholder="Tìm theo tên loại dịch vụ..."
                ...
                <div>
                    <button type="button" onclick="clearFilters()"
                        class="w-full px-4 lg:px-6 py-2 lg:py-2.5 bg-primary-100 text-primary-700 rounded-xl hover:bg-primary-200 font-semibold transition-colors text-sm lg:text-base">
                        Xóa bộ lọc
                    </button>
                </div>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full min-w-[600px]">
                <thead class="bg-primary-50">
                    <tr>
                        <th
                            class="px-3 lg:px-4 py-2 lg:py-3 text-left text-xs font-semibold text-primary-600 uppercase tracking-wider">
                            ID</th>
                        <th
                            class="px-3 lg:px-4 py-2 lg:py-3 text-left text-xs font-semibold text-primary-600 uppercase tracking-wider">
                            Tên loại dịch vụ</th>
                        ...
                        <th
                            ...
                            Trạng thái</th>
                        <th
                            class="px-3 lg:px-4 py-2 lg:py-3 text-right text-xs font-semibold text-primary-600 uppercase tracking-wider">
                            Thao tác</th>
                    </tr>
                </thead>
                    ...
                    <?php if (empty($service_types)): ?>
                        <tr id="noResultsRow">
                            <td colspan="5" class="px-3 lg:px-4 py-8 lg:py-12 text-center text-primary-500">
                                ...
                                <a href="?act=admin&module=service-types&action=create"
                                    class="text-accent hover:text-accent-dark font-semibold ml-1">Thêm mới</a>
                            </td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($service_types as $type): ?>
                            <tr class="service-type-row hover:bg-primary-50 transition-colors"
                                ...
                                data-id="<?= $type['id'] ?>">
                                <td class="px-3 lg:px-4 py-2 lg:py-3 text-xs lg:text-sm text-primary-600"><?= $type['id'] ?>
                                </td>
                                <td
                                    class="px-3 lg:px-4 py-2 lg:py-3 text-xs lg:text-sm font-semibold text-primary-700 service-type-name">
                                    <?= htmlspecialchars($type['name']) ?></td>
                                ...
                                <td class="px-3 lg:px-4 py-2 lg:py-3 text-xs lg:text-sm">
                                    <?php if ($type['status'] == 'active'): ?>
                                        <span
                                            class="px-2 lg:px-3 py-1 rounded-full text-xs font-bold uppercase bg-success-bg text-success-text">Hoạt
                                            động</span>
                                    <?php else: ?>
                                        <span
                                            class="px-2 lg:px-3 py-1 rounded-full text-xs font-bold uppercase bg-primary-100 text-primary-600">Vô
                                            hiệu</span>
                                    <?php endif; ?>
                                </td>
    ...
    <div class="mt-4 lg:mt-6 p-3 lg:p-4 bg-info-bg border-l-4 border-info rounded-xl">
        <p class="text-xs lg:text-sm text-info-text flex items-start gap-2">
            ...
            <span><strong>Lưu ý:</strong> Loại dịch vụ đã có services không thể xóa. Chỉ có thể chuyển trạng thái
                inactive.</span>
        </p>
    </div>
</div>
