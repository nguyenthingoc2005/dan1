...
<div class="bg-panel p-4 lg:p-6 rounded-2xl border-l-4 border-accent shadow-sm">
    <form method="POST" action="?act=admin&module=location-services&action=update-province" class="space-y-4 lg:space-y-6">
        ...
        <div>
            ...
            <div class="mb-3 lg:mb-4">
                <label class="block text-xs lg:text-sm font-semibold text-primary-700 mb-1 lg:mb-2">Quốc gia</label>
            ...
            <?php if (!empty($province['code'])): ?>
                <div class="mb-3 lg:mb-4">
                    <label class="block text-xs lg:text-sm font-semibold text-primary-700 mb-1 lg:mb-2">Mã tỉnh thành</label>
                    <input type="text" value="<?= htmlspecialchars($province['code']) ?>"
                        class="w-full px-3 lg:px-4 py-2 lg:py-2.5 bg-primary-100 border border-primary-200 rounded-xl text-primary-600 text-sm lg:text-base" readonly>
                    <p class="text-xs text-primary-500 mt-1">Mã tỉnh thành không thể thay đổi sau khi tạo</p>
                </div>
            <?php endif; ?>
