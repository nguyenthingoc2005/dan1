 ...
 * Variables: $service_types, $service_providers
 */
...
<div class="max-w-6xl mx-auto">
    ...
    <div class="mb-4 lg:mb-6">
        ...
        <h1 class="text-xl lg:text-2xl font-bold text-primary-700">Thêm dịch vụ mới</h1>
    </div>
    ...
    <form method="POST" action="?act=admin&module=services&action=store" class="bg-panel p-4 lg:p-6 rounded-2xl shadow-sm border border-primary-100 space-y-4 lg:space-y-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
            ...
            <div class="space-y-4 lg:space-y-6">
                ...
                <div>
                    ...
                    <small class="text-xs text-primary-500 mt-1">Bắt buộc phải chọn nhà cung cấp dịch vụ</small>
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" rows="4"
                        class="w-full px-3 lg:px-4 py-2 lg:py-2.5 bg-primary-50 border border-primary-100 rounded-xl focus:outline-none focus:border-accent focus:bg-white transition-all placeholder:text-primary-300 text-primary-700 text-sm lg:text-base"
                        placeholder="Mô tả chi tiết về dịch vụ..."><?= htmlspecialchars($old_input['description'] ?? '') ?></textarea>
                </div>
            </div>
            ...
            <div class="space-y-4 lg:space-y-6">
                ...
                <div>
                    ...
                    <select name="unit"
                        ...
                        foreach ($service_units as $value => $label):
                            ?>
                            <option value="<?= $value ?>" <?= ($old_input['unit'] ?? '') == $value ? 'selected' : '' ?>>
                ...
                <div>
                    ...
                    <select name="status"
                        class="w-full px-3 lg:px-4 py-2 lg:py-2.5 bg-primary-50 border border-primary-100 rounded-xl focus:outline-none focus:border-accent focus:bg-white transition-all text-primary-700 text-sm lg:text-base">
                        <option value="active" <?= ($old_input['status'] ?? 'active') == 'active' ? 'selected' : '' ?>>
                            Hoạt động
                        </option>
                        <option value="inactive" <?= ($old_input['status'] ?? '') == 'inactive' ? 'selected' : '' ?>>
                            Vô hiệu
                        </option>
                    </select>
                </div>
