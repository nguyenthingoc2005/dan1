...
<div class="max-w-4xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4 lg:mb-6">
        <h1 class="text-xl lg:text-2xl font-bold text-primary-700">Sửa mã giảm giá</h1>
    ...
    <form method="POST" action="?act=admin&module=discount-codes&action=update"
        ...
        <div>
            ...
            <input type="text" name="code" required value="<?= htmlspecialchars($discountCode['code']) ?>"
                class="w-full px-3 lg:px-4 py-2 lg:py-2.5 bg-primary-50 border border-primary-100 rounded-xl focus:outline-none focus:border-accent focus:bg-white transition-all text-primary-700 text-sm lg:text-base uppercase"
                maxlength="50"
        ...
        <div>
            ...
            <input type="text" name="name" value="<?= htmlspecialchars($discountCode['name'] ?? '') ?>"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div>
                ...
                <select name="discount_type" required id="discount_type"
                    ...
                    <option value="fixed" <?= $discountCode['discount_type'] == 'fixed' ? 'selected' : '' ?>>Số tiền cố định (VNĐ)</option>
                </select>
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    <input type="number" name="discount_value" required value="<?= htmlspecialchars($discountCode['discount_value']) ?>"
                        id="discount_value"
                        class="w-full px-3 lg:px-4 py-2 lg:py-2.5 bg-primary-50 border border-primary-100 rounded-xl focus:outline-none focus:border-accent focus:bg-white transition-all text-primary-700 text-sm lg:text-base"
                        min="0" step="0.01">
        ...
        <div>
            ...
            <input type="number" name="min_purchase" value="<?= htmlspecialchars($discountCode['min_purchase'] ?? '0') ?>"
                class="w-full px-3 lg:px-4 py-2 lg:py-2.5 bg-primary-50 border border-primary-100 rounded-xl focus:outline-none focus:border-accent focus:bg-white transition-all text-primary-700 text-sm lg:text-base"
                min="0" step="1000">
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div>
                ...
                <input type="date" name="start_date" value="<?= htmlspecialchars($discountCode['start_date'] ?? '') ?>"
                    class="w-full px-3 lg:px-4 py-2 lg:py-2.5 bg-primary-50 border border-primary-100 rounded-xl focus:outline-none focus:border-accent focus:bg-white transition-all text-primary-700 text-sm lg:text-base">
            </div>
            ...
            <div>
                ...
                <input type="date" name="end_date" value="<?= htmlspecialchars($discountCode['end_date'] ?? '') ?>"
                    class="w-full px-3 lg:px-4 py-2 lg:py-2.5 bg-primary-50 border border-primary-100 rounded-xl focus:outline-none focus:border-accent focus:bg-white transition-all text-primary-700 text-sm lg:text-base">
            </div>
        </div>
        ...
        <div>
            ...
            <input type="number" name="usage_limit" value="<?= htmlspecialchars($discountCode['usage_limit'] ?? '0') ?>"
                class="w-full px-3 lg:px-4 py-2 lg:py-2.5 bg-primary-50 border border-primary-100 rounded-xl focus:outline-none focus:border-accent focus:bg-white transition-all text-primary-700 text-sm lg:text-base"
                min="0" step="1">
            <small class="text-xs text-primary-500 mt-1 block">
                Đã sử dụng: <?= $discountCode['used_count'] ?> lần
            </small>
        </div>
        ...
        <div>
            ...
            <select name="status"
                class="w-full px-3 lg:px-4 py-2 lg:py-2.5 bg-primary-50 border border-primary-100 rounded-xl focus:outline-none focus:border-accent focus:bg-white transition-all text-primary-700 text-sm lg:text-base">
                <option value="active" <?= ($discountCode['status'] ?? 'active') == 'active' ? 'selected' : '' ?>>Hoạt động</option>
                <option value="inactive" <?= ($discountCode['status'] ?? '') == 'inactive' ? 'selected' : '' ?>>Vô hiệu</option>
            </select>
        </div>
