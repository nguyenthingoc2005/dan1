...
<div class="max-w-6xl mx-auto">
    ...
    <form method="POST" action="?act=staff-tours&action=update" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="p-4 lg:p-5">
                ...
                <div class="mt-5">
                    ...
                    <select name="status" class="w-full md:w-1/2 px-3 py-2 border rounded focus:border-accent focus:outline-none">
                        ...
                        <option value="pending" <?= $tour['status'] == 'pending' ? 'selected' : '' ?>>Chờ duyệt (Pending)</option>
                    </select>
                    <p class="text-xs text-gray-500 mt-1">Staff chỉ có thể sửa tour ở trạng thái Nháp hoặc Chờ duyệt.</p>
                </div>
            </div>
        </div>
            ...
            <div class="p-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <?php if (!empty($policies)): ?>
                        ...
                        <?php endforeach; ?>
                    <?php else: ?>
                        <p class="text-gray-500 text-sm">Chưa có chính sách nào. <a href="?act=staff-tours&action=createPolicy" class="text-accent hover:underline">Tạo mới</a></p>
                    <?php endif; ?>
                </div>
            </div>
        </div>
...
<script>
    ...
    function openAddServiceModal(dayNumber) {
        ...
        fetch(`?act=staff-tours&action=loadDayServicesEditor&day=${dayNumber}&tour_id=<?= $tour['id'] ?>`)
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    // Hiển thị modal hoặc load vào container
                    // Tạm thời: redirect đến trang loadDayServicesEditor
                    window.open(`?act=staff-tours&action=loadDayServicesEditor&day=${dayNumber}&tour_id=<?= $tour['id'] ?>`, '_blank');
                }
            })
            .catch(error => {
