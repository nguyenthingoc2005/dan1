...
<script>
    ...
    function initTinyMCEForItinerary(dayNumber, savedContent = null) {
        ...
        try {
            tinymce.init({
                ...
                images_upload_url: '?act=staff-tours&action=uploadImage',
    ...
    function loadItineraryManagerForDay(dayNumber) {
        ...
        // Build URL
        let url = `?act=staff-tours&action=loadItineraryManager&day=${dayNumber}&step=${step}`;
    ...
    function loadDayServicesForDay(dayNumber) {
        ...
        let url = `?act=staff-tours&action=loadDayServicesEditor&day=${dayNumber}`;
        const tourId = document.querySelector('input[name="tour_id"]')?.value || '';
        if (tourId) {
            url += `&tour_id=${tourId}`;
        }
    ...
    function loadServiceInfoForDay(dayNumber) {
        ...
        let url = `?act=staff-tours&action=getServiceInfo&id=${serviceId}`;
        if (tourStartDate) {
    ...
    function saveFormDataToSession(additionalData = {}) {
        ...
        fetch('?act=staff-tours&action=saveFormSession', {
            method: 'POST',
            headers: {
