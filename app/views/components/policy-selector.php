        ...
        <?php else: ?>
            <div class="space-y-2">
                <?php foreach ($selected_policy_ids as $policy_id): ?>
                    ...
                    if ($policy):
                    ?>
                        <div class="selected-policy-item bg-green-50 border border-green-200 rounded-lg p-3 flex items-center justify-between">
                            <div>
                            ...
                            <button type="button" 
                                    onclick="removePolicy(<?= $policy_id ?>)"
            ...
            <div class="policy-type-group">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <?php foreach ($type_policies as $policy): ?>
                        ...
                        <div class="policy-card border rounded-lg p-4 <?= $is_selected ? 'bg-blue-50 border-blue-300' : 'bg-white border-gray-200 hover:border-blue-300' ?> transition-colors">
                                ...
                                <div class="flex-1">
                                    <label for="policy-<?= $policy['id'] ?>" class="cursor-pointer">
                                        ...
                                        <?php if (!empty($policy['description'])): ?>
                                            <div class="text-sm text-gray-600 mt-1"><?= htmlspecialchars($policy['description']) ?></div>
                                        <?php endif; ?>
                                    </label>
                                    
                                    <button type="button" 
                                            onclick="previewPolicy(<?= $policy['id'] ?>)"
        ...
        <form id="create-policy-form" class="p-6" onsubmit="saveNewPolicy(event); return false;">
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea id="new-policy-description"
                              rows="2"
                              class="w-full px-3 py-2 border rounded focus:border-purple-500"
                ...
                <div>
                    ...
                    <textarea id="new-policy-content"
                              rows="8"
                              required
                              class="w-full px-3 py-2 border rounded focus:border-purple-500"
...
<div id="preview-policy-modal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
    <div class="bg-white rounded-lg shadow-xl max-w-3xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div class="p-6 border-b">
        ...
        <div class="p-6 border-t flex justify-end">
            <button type="button" 
                    onclick="closePreviewPolicyModal()"
...
<script>
const selectedPolicies = <?= json_encode(array_flip($selected_policy_ids)) ?>;
...
function saveNewPolicy(event) {
    ...
    const formData = {
        name: document.getElementById('new-policy-name').value,
        ...
        content: document.getElementById('new-policy-content').value
    };
