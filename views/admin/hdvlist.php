                                        ...
                                        <tr>
                                                ...
                                                <div class="d-flex align-items-center">
                                                    ...
                                                    <div>
                                                        <div class="fw-bold text-dark">
                                                            <?= htmlspecialchars($hdv['ho_ten'] ?? 'Chưa cập nhật') ?></div>
                                                        <div class="small text-muted">
                                                            ...
                                                            <?php if (isset($hdv['ngay_sinh'])): ?>
                                                                <span><i class="bi bi-cake2"></i>
                                                                    <?= date('d/m/Y', strtotime($hdv['ngay_sinh'])) ?></span>
                                                            <?php endif; ?>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <td>
                                                <div class="d-flex flex-column gap-1">
                                                    <span class="text-dark small fw-medium"><i
                                                            class="bi bi-telephone me-2 text-primary"></i>
                                                        <?= htmlspecialchars($hdv['so_dien_thoai'] ?? '---') ?></span>
                                                    <span class="text-muted small"><i class="bi bi-envelope me-2"></i>
                                                        <?= htmlspecialchars($hdv['email'] ?? '---') ?></span>
                                                    <span class="text-muted small text-truncate" style="max-width: 200px;"
                                                        title="<?= htmlspecialchars($hdv['dia_chi_lien_he'] ?? '') ?>">
                                                        <i class="bi bi-geo-alt me-2"></i>
                                                        <?= htmlspecialchars($hdv['dia_chi_lien_he'] ?? 'Chưa có địa chỉ') ?>
                                                    </span>
                                                </div>
                                            </td>
                                            ...
                                            <td>
                                                <div class="mb-1">
                                                    <span class="badge badge-soft badge-soft-primary"><i
                                                            class="bi bi-translate me-1"></i>
                                                        <?= htmlspecialchars($hdv['ngon_ngu_su_dung'] ?? 'Chưa rõ') ?></span>
                                                </div>
                                                <div class="text-muted text-truncate-2 small"
                                                    title="<?= htmlspecialchars($hdv['kinh_nghiem_lam_viec'] ?? '') ?>">
                                                    <?= htmlspecialchars($hdv['kinh_nghiem_lam_viec'] ?? 'Chưa cập nhật kinh nghiệm') ?>
                                                </div>
                                                <?php if (!empty($hdv['chung_chi_chuyen_mon'])): ?>
                                                    <div class="text-dark small text-truncate-2"
                                                        title="Chứng chỉ: <?= htmlspecialchars($hdv['chung_chi_chuyen_mon']) ?>">
                                                        <i class="bi bi-award-fill me-1 text-warning"></i>
                                                        <?= htmlspecialchars($hdv['chung_chi_chuyen_mon']) ?>
                                                    </div>
                                                <?php endif; ?>
                                            </td>
                                            ...
                                            <td>
                                                ...
                                                if ($status == 'Bận' || $status == 'Đang dẫn tour')
                                                    $statusClass = 'badge-soft-warning';
                                                if ($status == 'Tạm ngưng')
                                                    $statusClass = 'badge-soft-danger'; // Đỏ (nếu bạn định nghĩa css này)
                                                ?>
                                            ...
                                            <td class="text-end pe-4">
                                                <div class="d-flex justify-content-end gap-2">
                                                    <a href="index.php?act=edithdv&id=<?= $hdv['hdv_id'] ?>"
                                                        class="btn-icon edit" title="Sửa thông tin">
                                                        <i class="bi bi-pencil-square"></i>
                                                    </a>

                                                    <a href="index.php?act=deletehdv&id=<?= $hdv['hdv_id'] ?>"
